...
import { getProgressColor } from "@/utils/password-utils";
...
export default function PasswordGeneratorForm() {
  ...
  const { generate, generatePassphrase, isGenerating, generatedPassword, strength, copyToClipboard } = usePasswordGenerator();

  const handleGenerate = () => {
    if (form.passphraseMode) {
      const wordCount = Math.max(4, Math.min(8, Math.round(form.length / 8)));
      generatePassphrase({
        words: wordCount,
        separator: "-",
      });
    ...
    <div className="space-y-6 text-left">
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center text-xs font-semibold text-gray-700 dark:text-gray-300">
          <span>{form.passphraseMode ? `Approximate Words: ${Math.max(4, Math.min(8, Math.round(form.length / 8)))}` : `Password Length: ${form.length} characters`}</span>
      ...
      {!form.passphraseMode ? (
        ...
        <div className="rounded-xl border border-blue-200/50 bg-blue-500/5 p-4 dark:border-blue-900/30 text-xs text-blue-800 dark:text-blue-300 flex items-start gap-2">
          ...
          <p>
            Passphrase mode generates sequences of words fetched securely from the backend. They are easy to memorize but extremely difficult for computer grids to crack.
          </p>
        </div>
      )}
      ...
      {strength && (
        <div className="rounded-2xl border border-gray-200/60 p-4 space-y-3 dark:border-gray-800">
          ...
          <div className="grid grid-cols-2 gap-4 pt-2 text-xs">
            ...
            <div>
              <span className="text-gray-400 dark:text-gray-500">Structural Category:</span>
              <p className="font-bold text-gray-900 dark:text-white text-sm">{strength.label}</p>
            </div>
          </div>
        </div>
      )}
